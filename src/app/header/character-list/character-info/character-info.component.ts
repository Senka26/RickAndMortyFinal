...
export class CharacterInfoComponent implements OnInit {
  ...
  private loadFavouriteCharacters() {
    const favouritesFromStorage = localStorage.getItem('favouriteCharacters');
    this.favouriteCharacters = favouritesFromStorage
      ? JSON.parse(favouritesFromStorage)
      : [];
  }

  removeFromFavourites() {
    this.favouriteCharacters = this.favouriteCharacters.filter(
      (char) => char.id !== this.data.id
    );
    localStorage.setItem(
      'favouriteCharacters',
      JSON.stringify(this.favouriteCharacters)
